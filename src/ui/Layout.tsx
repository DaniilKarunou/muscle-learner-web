...
export default function Layout({ children }: LayoutProps) {
    ...
    return (
        <div className="relative min-h-screen pb-[calc(7rem+env(safe-area-inset-bottom))]">
            ...
            <header className="sticky top-0 z-30 border-b border-white/60 bg-white/72 pt-[env(safe-area-inset-top)] backdrop-blur-xl">
                <div className="mx-auto flex w-full max-w-6xl items-center justify-between gap-4 px-4 py-4 sm:px-6">
                    ...
                    <div className="rounded-full border border-atlas-100 bg-white/80 px-3 py-2 text-xs font-semibold text-ink-500">
                        {installPrompt.standalone ? "Tryb aplikacji" : "Mobile-first nauka anatomii"}
                    </div>
                </div>
            </header>

            <main className="mx-auto flex w-full max-w-6xl flex-col gap-4 px-4 pt-6 sm:px-6">
                {!isQuizRoute && installPrompt.visible ? (
                    <InstallAppBanner
